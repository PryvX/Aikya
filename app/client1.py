...
df = pd.read_csv("data/AA1.csv")

st.title("🏦 Account Aggregator 1")
st.image("Saafe-AA1.png", width=200)
    ...
    if st.button("▶️ Start API Server"):
        if st.session_state.api_process is None:
            process = subprocess.Popen(
                ["python", "-m", "uvicorn", "lender1:app", "--host", "0.0.0.0", "--port", "8001"]
            )
            st.session_state.api_process = process
            st.success("✅ API Server started on port 8001")
        else:
